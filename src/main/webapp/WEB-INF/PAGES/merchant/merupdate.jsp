...
<script>
    ...
    $('#updInfo').form('load',{
            ID:row[0].ID,
        merId:row[0].MerchantID,
        merName:row[0].MerchantName,
        SupportPaymentWay:row[0].SupportPaymentWay,
        Authcode:row[0].Authcode
    });
    $('#stat').val(row[0].Status);
    $('#updInfo').form({
        url: '/merchant/updInfo',
        onSubmit: function () {
        ...
        success: function (data) {
            if (data== "1") {
                ...
                $.messager.alert('提示信息', "修改失败,原因:" + data.error);
            }
        }
    });
